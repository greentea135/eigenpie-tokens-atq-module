...
const SUBGRAPH_URLS: Record<string, { decentralized: string }> = {
  ...
  "1": {
    decentralized:
      "https://gateway.thegraph.com/api/[api-key]/subgraphs/id/BbkLSUBpyJv8UiCyMyQBGpngz8qfdhx4LJBqFdHqiwhH",
  },
};
...
function transformPoolsToTags(chainId: string, pools: Pool[]): ContractTag[] {
  ...
  return validPools.map((pool) => {
    ...
    return {
      ...
      "Project Name": "Eigenpie",
      "UI/Website Link": "https://www.eigenlayer.magpiexyz.io/",
      "Public Note": `Eigenpie's ${pool.outputToken.symbol} (${pool.outputToken.name}) token contract.`,
    };
  });
}
